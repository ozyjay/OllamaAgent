...
final class OllamaAPIClientTests: XCTestCase {
    ...
    func testAgentSectionsUseCoreMoreOrder() {
        XCTAssertEqual(AgentSection.allCases.map(\.rawValue), [
            "Logs",
            "Profiles",
            "Models",
            "Settings"
        ])
        XCTAssertEqual(AgentSection.allCases.map(\.systemImage), [
            "doc.text.magnifyingglass",
            "slider.horizontal.3",
            "shippingbox",
            "gearshape"
        ])
        XCTAssertTrue(AgentSection.allCases.allSatisfy { !$0.navigationTitle.isEmpty })
        XCTAssertTrue(AgentSection.allCases.allSatisfy { !$0.menuTitle.isEmpty })
    }

    @MainActor
    func testAgentNavigationDefaultsToLogs() {
